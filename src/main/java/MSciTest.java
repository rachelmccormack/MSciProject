...
public class MSciTest {
    ...
    public static void start(String name) throws Exception {
       /* Scanner reader = new Scanner(System.in);  // Reading from System.in
        ...
        double[] mean = new double[runs];
        for (int times = 0; times < runs; times++) {
            outscore[0] = 0;
            outscore[1] = 0;
            double replacement = gaussian(getFeatures()[0][doc][feat]);
            for (int x = 0; x < 64; x++){
                mean[times] = mean[times] + getFeatures()[0][doc][x];
            }
            mean[times] = mean[times]/64;
            ...
            applyModel(2, 64, getFeatures()[0], outscore);
            threshold.put(mean[times], outscore[oppositeDoc]);
            System.out.println(mean[times]);
            results.put( mean[times], outscore[doc]);
    ...
    private static XYDataset createXYDataset() throws FileNotFoundException {
        ...
        for (Map.Entry<Double, Double> entry : threshold.entrySet()) {
            System.out.println(entry.getKey());
            System.out.println(entry.getValue());
    ...
    public static JFreeChart test(String applicationTitle, String chartTitle, String name) throws IOException {
        ...
        File image = new File("mean/chart" + name + ".png");
